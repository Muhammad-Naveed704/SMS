...
import { useTeacherTimetable } from "@/features/teacher/hooks/useTeacherQueries";
...
export function TimetablePage() {
  const { data: slots = [], isLoading, isError, refetch } = useTeacherTimetable();
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: "Teacher", href: "/teacher/dashboard" }, { label: "Timetable" }]} />
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full min-w-[800px] text-sm">
          ...
          <tbody>
            {PERIODS.map((period) => {
              const timeSlot = slots.find((s) => s.period === period);
              return (
                <tr key={period} className="border-b border-border last:border-0">
                  <td className="px-4 py-3 font-medium text-muted-foreground">
                    P{period}
                    {timeSlot && <span className="block text-xs">{timeSlot.startTime}</span>}
                  </td>
                  {DAYS.map((day) => {
                      ...
                      <td key={day} className="px-2 py-2">
                        {slot ? (
                          <Card padding="sm" className="bg-primary/5">
                            ...
                            <p className="text-xs text-muted-foreground">{slot.className}-{slot.section}</p>
